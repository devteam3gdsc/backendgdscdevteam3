...
const commentsController = {
    ...
    createComment: async (req,res)=>{
       try {
        const { avatar,authorname,text,code } = req.body;
    ...
    deleteComment: async (req,res)=>{
        try {
            ...
            const postId = req.params.postId;
            const comment = await Comments.deleteOne({author:userId,_id:commentId});
            await Post.findByIdAndUpdate(postId,{$inc:{totalComments:-1}});
    ...
    editComment: async (req,res)=>{
        try {
            ...
            const comment = await Comments.updateOne({author:userId,_id:commentId},{$set:{text:text,code:code}});
