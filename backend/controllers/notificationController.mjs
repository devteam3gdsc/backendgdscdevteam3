...
const notificationController = {
  ...
  createLikeNotification: async (req, res) => {
    try {
      ...
      const notification = await notificationServices.createLikeNotification({
        postId,
        senderId,
      });
      res.status(201).json(notification);
  ...
  createCommentNotification: async (req, res) => {
    try {
      ...
      const notification = await notificationServices.createCommentNotification(
        { commentId, senderId },
      );
      res.status(201).json(notification);
    } catch (error) {
  ...
  getUserNotifications: async (req, res) => {
    try {
      ...
      console.error(
        `Error in getUserNotifications controller: ${error.message}`,
      );
      res.status(500).json({
        message:
  ...
  deleteNotification: async (req, res) => {
    try {
      ...
      const deletedNotification = await notificationServices.deleteNotification(
        req.params.notificationId,
      );
