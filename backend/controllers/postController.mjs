...
const postController = {
  //[GET] /me/:id?page=...&limit=...&search=...
   getUserPost : async (req,res)=>{
        try {
            ...
            const Data = await Post.aggregate([
                {$match:{$and:[{$or:[{author:userId},{stored:userId}]},{title:{$regex:search,$options:"i"}}]}},
            ...
            if (((!Data[0].countingPostsNoTags[0])&&!tags[0])||((!Data[0].countingPostsWithTags[0])&&tags[0])){
                return res.status(500).json("Oops!There is no posts here!")
            }
            const totalPosts = tags[0]?Data[0].countingPostsWithTags[0].totalPosts:Data[0].countingPostsNoTags[0].totalPosts;
   ...
   createPost : async (req,res)=>{
    try {
        const {title, content, visibility, stored, authorname, avatar} = req.body;
   ...
   getCommunityPosts: async (req,res)=>{
        try {
            ...
            const Data = await Post.aggregate([
                 {$match:{$and:[{visibility:'public'},{title:{$regex:search,$options:"i"}}]}},
               {$sort:{[sortValue]:sortOrder}},
               {$facet: 
               {
                   posts: [
...
// },
   // /post/store/:postId
   storePost: async (req,res)=>{//cần xem lại, nếu người đó là tác giả hay đã từng lưu?,làm cho ẩn đi khi gửi
        try {
            const userId = req.user.id;
   ...
   //detail/:postId
   detailPost: async (req,res)=>{//thieu gioi han comment
    try {
   ...
   editPost: async (req,res)=>{
    try {
        ...
        const codeFiles = req.files['code_files'];
        const files = codeFiles.map(file => file.path);
        const post = await Post.updateOne({author:userId,_id:postId},{$set:{title:title,content:content,files:files}});
   ...
   likePost: async (req,res) =>{
    try {
        const userId = req.user.id;
        const postId = req.params.postId;
        await Post.findByIdAndUpdate(postId, {$addToSet:{likes:userId},$inc:{totalLikes:1}});
   ...
   unLikePost: async (req,res) =>{
    try {
        const userId = req.user.id;
        const postId = req.params.postId;
        await Post.findByIdAndUpdate(postId, {$pull:{likes:userId},$inc:{totalLikes:-1}});
