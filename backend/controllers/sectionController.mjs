    ...
    getUsers: async (req,res)=>{
      ...
      getUninvitedUsers: async (req,res)=>{
        try {
          ...
          const projectMembers = (await findDocument(Project,{_id:section.project},{members:1,_id:0})).members
          const projectMembersId = projectMembers.map((member)=>{return member.users})
          const page = req.query.page || 1;
          const limit = req.query.limit || 5;
          const skip = (page-1)*limit;
          const order = req.query.order || "descending";
          ...
          const matchData = [{_id:{$in:projectMembersId}},{_id:{$nin:section.participants}}]
          if (search){
            matchData.push({displayname:{$regex:search,$option:"i"}})
          }
          const result = await userServices.getUsers(userId,matchData,sortValue,sortOrder,skip,limit);
      ...
      getSectionsPosts: async (req,res) =>{
        try {
            ...
            const section = await findDocument(Section,{_id:sectionId},{name:1,_id:0});
            const me = await User.findById(userId);
            const newRecent = me.recent.filter((pin)=>{return pin.name !== section.name});
            newRecent.push({
              id:sectionId,
              recentType:"section",
              name: section.name,
