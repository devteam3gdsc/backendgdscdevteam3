...
const userController = {
  ...
  getPin: async (req,res)=>{
    try {
      const userId = new mongoose.Types.ObjectId(`${req.user.id}`);
      const pins = (await findDocument(User,{_id:userId},{pins:1,_id:0})).pins
  ...
  unPin: async (req,res)=>{
    try {
      ...
      const pins = (await findDocument(User,{_id:userId},{pins:1,_id:0})).pins
      await User.updateOne({_id:userId},{$pull:{pins:pins[position - 1]}})
  ...
  getRecent: async (req,res)=>{
    try {
      const userId = new mongoose.Types.ObjectId(`${req.user.id}`);
      const result = (await findDocument(User,{_id:userId},{recent:1,_id:0})).recent
      return res.status(200).json(result)
    } catch (error) {
      if (error instanceof httpError)
        return res.status(error.statusCode).json(error.message);
