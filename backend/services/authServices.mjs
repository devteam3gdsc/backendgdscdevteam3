...
const authServices = {
    passwordCheck: async (password, hashedPassword) => await bcrypt.compare(password,hashedPassword),
    ...
    login: async({...data})=>{
        ...
        if (await authServices.passwordCheck(password,user.password)){
            ...
            const newRefreshToken = tokensAndCookies.createNewRefreshToken(user._id);
            ...
            else {throw new httpError(`Login services error: ${error.message || error}`, 500);}
        }   
    },
    logout:async(refreshToken)=>{
        try {
            if(!refreshToken) throw new httpError("No refresh Token provided!",404);
            await updateDocument(User,1,[{refreshTokens:{$in:[refreshToken]}}],[{$pull:{refreshTokens:refreshToken}}]);
            return new httpResponse("Logged Out!",200);
        } catch (error) {
            if (error instanceof httpError) throw error
            else {throw new httpError(`loggout services error: ${error.message||error}`, 500);}
        }
    },
    requestRefreshToken: async(refreshToken)=>{
        try {
            if(!refreshToken) throw new httpError("refreshToken is required",404);
            const userId = tokensAndCookies.refreshTokenDecoding(refreshToken);
            ...
            let userRefreshTokens = await findDocument(User,1,[{_id:userId}],[{refreshToken:1}]);
            console.log(userRefreshTokens);
            userRefreshTokens = userRefreshTokens.filter((token)=>token !== refreshToken);
            userRefreshTokens.push(newRefreshToken);
            await updateDocument(User,1,[{_id:userId}],[{$set:{refreshTokens:userRefreshTokens}}])
            return {
                newAccessToken:newAccessToken,
                newRefreshToken:newRefreshToken,
            };
            ...
            if (error instanceof httpError) throw error
            else {throw new httpError(`refreshtoken services error: ${error.message || error}`, 500);}
        }
    }
}
