...
const postServices = {
    getPosts: async(userId,{...matchData},criterias,orders,skip,limit)=>{
        try {
    ...
    if (data.tags) {
      ...
      const userData = await findDocument(User,1,[{_id:userId}],[{avatar:1,displayname:1}]);
      ...
      const newPost = await Post.create({
        ...
        editedAt: Date.now(),
      }).catch((error)=>{throw new httpError(`creating post failed: ${error}`,500)})
      return newPost._id
    } catch (error) {
