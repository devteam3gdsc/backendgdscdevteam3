...
const sectionServices = {
    createSection: async ({...data})=>{
        try {
            const id = data.parentId?new mongoose.Types.ObjectId(`${data.parentId}`):new mongoose.Types.ObjectId(`${data.projectId}`);
            if (data.parentId){
                const parentSection = await Section.findById(id);
                if (!parentSection) throw new httpError("cant find the parent Section",404)
