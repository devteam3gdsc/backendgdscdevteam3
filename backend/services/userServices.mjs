...
const userServices = {
  ...
  updateUserFullInfo: async (userId,avatarFile,{displayname,...updatedData})=>{
        try {
        const user = await findDocument(User,1,[{_id:userId}],[])
        console.log(user)
        ...
        await Post.updateMany({author:userId},{$set:{ authorname: displayname, avatar: avatarURL }})
        await Comments.updateMany({author:userId},{$set:{ authorname: displayname, avatar: avatarURL }})
        return new httpResponse("updated successfully",200)
        } catch (error) {
          if (error instanceof httpError) throw error
          else throw new httpError(`updateFullUserInfo service error:${error}`,500)
        }
    },
  signUpUser: async ({...data})=>{
    ...
    if (user) throw new httpError("Username of email has already existed",400)
    else {
